...
bot.on("new_chat_members", async (ctx) => {
  if (ctx.update.message.chat.id.toString() !== "-1001803220721") return;
  ...
  if (!foundUser) {
    if (ctx.message.message_id) {
      ctx.telegram.sendMessage(
        ctx.message.chat.id,
        `Hãy tham gia @chatbot_openai trước khi sử dụng bot!`,
        {
          reply_to_message_id: ctx.message.message_id,
        }
      );
    } else {
      ctx.sendMessage(`Hãy tham gia @chatbot_openai trước khi sử dụng bot!`);
    }
